...
class Config(event: FMLPreInitializationEvent) {
  ...
  val COMMENT = String.format("The sound types to use as the ping noise. Can be any of the " +
  ...
  var soundType = config.get("Sound Type", "The sound type", DEFAULT_SOUND).getString
  ...
  val customNamesString = config.get("Custom Names", "Custom names to be pinged as", "").getString
  val customNames = getNamesFromConfig(customNamesString)
  ...
  val customColor = getColorFromConfig(config.get("Custom Color", "The custom color for the highlighted text", "").getString)
  if (customColor.isEmpty) FMLLog.warning("[Ping!] Highlight color is invalid. Please fix your config.")

  val bold = config.getBoolean("Bold", "Chat Style", true, "")
  val italic = config.getBoolean("Italic", "Chat Style", false, "")
  val strikethrough = config.getBoolean("Strikethrough", "Chat Style", false, "")
